...
inline std::string windowsCpuNameFromRegistry()
{
        ...
        ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueExA(hKey, "ProcessorNameString", nullptr, &type, reinterpret_cast<LPBYTE>(data), &size) ==
            ERROR_SUCCESS)
        {
            if (type == REG_SZ)
            {
...
inline std::optional<double> windowsCpuMHzFromRegistry()
{
        ...
        ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueExA(hKey, "~MHz", nullptr, &type, reinterpret_cast<LPBYTE>(&mhz), &size) == ERROR_SUCCESS)
        {
            RegCloseKey(hKey);
...
template <typename T> inline bool sysctlValue(const char *name, T &out)
{
    size_t len = sizeof(T);
...
inline CpuInfo queryCpuInfo()
{
    ...
    if (sysctlString("machdep.cpu.brand_string", brand))
        ci.model = brand;
    if (sysctlString("machdep.cpu.vendor", vendor))
        ci.vendor = vendor;
    if (sysctlString("machdep.cpu.features", feats))
        ci.featuresSummary = feats;
...
inline CpuInfo queryCpuInfo()
{
    ...
    ci.logicalProcessors = (n > 0 ? static_cast<int>(n) : static_cast<int>(std::thread::hardware_concurrency()));
#else
    ci.logicalProcessors = static_cast<int>(std::thread::hardware_concurrency());
#endif
    ...
    if (cpuinfo)
    {
        ...
        while (std::getline(cpuinfo, line))
        {
            ...
            if (key == "physical id")
            {
                try
                {
                    physicalIds.insert(std::stoi(val));
                }
                catch (...)
                {
                }
            ...
            if (key == "cpu cores")
            {
                try
                {
                    cpuCoresPerSocket = std::stoi(val);
                }
                catch (...)
                {
                }
        ...
        if (!physicalIds.empty())
            ci.sockets = static_cast<int>(physicalIds.size());
        ci.physicalCores = (ci.sockets && cpuCoresPerSocket) ? std::optional<int>(*ci.sockets * *cpuCoresPerSocket)
                                                             : cpuCoresPerSocket;
...
inline void logCpuOverview()
{
    ...
    if (!ci.featuresSummary.empty())
    {
        ...
        std::string flags =
            ci.featuresSummary.size() > MAX_LEN ? (ci.featuresSummary.substr(0, MAX_LEN) + " ...") : ci.featuresSummary;
