...
namespace star {
	...
	public:
		static void registerCallbacks(std::function<void(core::device::DeviceContext&, const int&, const vk::Extent2D&)> initCallback, 
			std::function<void(core::device::DeviceContext&)> destroyCallback);

		static void registerLoadGeomDataCallback(std::function<std::pair<std::unique_ptr<StarBuffers::Buffer>, std::unique_ptr<StarBuffers::Buffer>>(core::device::DeviceContext&, BufferHandle, BufferHandle)>);
	...
	private: 
		static std::stack<std::function<void(core::device::DeviceContext&, const int&, const vk::Extent2D&)>> initCallbacks;
		static std::stack<std::function<void(core::device::DeviceContext&)>> destroyCallbacks;
		static std::stack<std::function<std::pair<std::unique_ptr<StarBuffers::Buffer>, std::unique_ptr<StarBuffers::Buffer>>(core::device::DeviceContext&, BufferHandle, BufferHandle)>> loadGeometryCallbacks;
		...
		static void runInits(core::device::DeviceContext& device, const int& numFramesInFlight, const vk::Extent2D& screensize);
