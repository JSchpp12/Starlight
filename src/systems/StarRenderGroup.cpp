...
void StarRenderGroup::recordCommands(vk::CommandBuffer& commandBuffer, int swapChainImageIndex) {
	int vertexCount = 0;

	for (auto& group : this->groups) {
		group.baseObject.object.recordCommands(commandBuffer, pipelineLayout, swapChainImageIndex, group.baseObject.startVBIndex, group.baseObject.startIBIndex); 
		for (auto& obj : group.objects) {
			//record commands for each object
			obj.object.recordCommands(commandBuffer, this->pipelineLayout, swapChainImageIndex, obj.startVBIndex, obj.startIBIndex);
		}
	}
}
