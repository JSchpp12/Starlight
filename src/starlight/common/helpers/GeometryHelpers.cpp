...
void star::GeometryHelpers::packTriangleAdjacency(std::vector<Vertex> &verts, std::vector<uint32_t> &indices)
{
    ...
    {
        triangles.push_back(TriangleTracker{EdgeTracker(uniqueIndicies[i], uniqueIndicies[i + 1]),
...
void star::GeometryHelpers::calculateAndApplyVertTangents(std::vector<star::Vertex> &verts,
                                                          std::vector<uint32_t> &indices)
{
    ...
    for (size_t i = 0; i < indices.size() - 3; i += 3)
    {
        ...
        std::array<Vertex *, 3> triVerts{&verts.at(indices.at(i)), &verts.at(indices.at(i + 1)),
                                         &verts.at(indices.at(i + 2))};
        calcTangentSpaceVectors(triVerts);
    }
}

void star::GeometryHelpers::calculateAxisAlignedBoundingBox(const glm::vec3 lowerBound, const glm::vec3 upperBound,
                                                            std::vector<star::Vertex> &vertList,
                                                            std::vector<uint32_t> &indicesList, bool lineList)
{
    glm::vec3 center{(upperBound.x + lowerBound.x) / 2, (upperBound.y + lowerBound.y) / 2,
                     (upperBound.z + lowerBound.z) / 2};

    vertList = std::vector<Vertex>{Vertex{.pos = lowerBound, .normal = lowerBound - center},
                                   Vertex{.pos = glm::vec3{upperBound.x, lowerBound.y, lowerBound.z},
                                          .normal = glm::vec3{upperBound.x, lowerBound.y, lowerBound.z} - center},
                                   Vertex{.pos = glm::vec3{upperBound.x, lowerBound.y, upperBound.z},
                                          .normal = glm::vec3{lowerBound.x, upperBound.y, lowerBound.z} - center},
                                   Vertex{.pos = glm::vec3{lowerBound.x, lowerBound.y, upperBound.z},
                                          .normal = glm::vec3{upperBound.x, upperBound.y, lowerBound.z} - center},
                                   Vertex{.pos = glm::vec3{lowerBound.x, upperBound.y, lowerBound.z},
                                          .normal = glm::vec3{lowerBound.x, upperBound.y, lowerBound.z} - center},
                                   Vertex{.pos = glm::vec3{upperBound.x, upperBound.y, lowerBound.z},
                                          .normal = glm::vec3{upperBound.x, upperBound.y, lowerBound.z} - center},
                                   Vertex{.pos = upperBound, .normal = upperBound - center},
                                   Vertex{.pos = glm::vec3{lowerBound.x, upperBound.y, upperBound.z},
                                          .normal = glm::vec3{lowerBound.x, upperBound.y, upperBound.z} - center}};

    if (lineList)
        indicesList = std::vector<uint32_t>{// base
                                            0, 1, 1, 2, 2, 3, 3, 0,
                                            // middle
                                            0, 4, 1, 5, 2, 6, 3, 7,
                                            // top
        ...
        indicesList = std::vector<uint32_t>{// L
                                            1, 0, 4, 1, 4, 5,
                                            // Back
                                            2, 1, 5, 2, 5, 6,
                                            // R
