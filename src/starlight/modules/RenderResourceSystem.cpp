...
// std::stack<std::function<void(star::core::device::DeviceContext &, const int &, const vk::Extent2D &)>>
//     star::RenderResourceSystem::initCallbacks =
//         std::stack<std::function<void(core::device::DeviceContext &, const int &, const vk::Extent2D &)>>();
// std::stack<std::function<void(star::core::device::DeviceContext &)>> star::RenderResourceSystem::destroyCallbacks =
//     std::stack<std::function<void(star::core::device::DeviceContext &)>>();
// std::stack<
//     std::function<std::pair<std::unique_ptr<star::StarBuffers::Buffer>, std::unique_ptr<star::StarBuffers::Buffer>>(
//         star::core::device::DeviceContext &, star::BufferHandle, star::BufferHandle)>>
//     star::RenderResourceSystem::loadGeometryCallbacks =
//         std::stack<std::function<std::pair<std::unique_ptr<StarBuffers::Buffer>, std::unique_ptr<StarBuffers::Buffer>>(
//             core::device::DeviceContext &, BufferHandle, BufferHandle)>>();
// std::stack<std::function<void(const uint32_t &, const uint32_t &, const uint32_t &)>>
//     star::RenderResourceSystem::geometryDataOffsetCallbacks =
//         std::stack<std::function<void(const uint32_t &, const uint32_t &, const uint32_t &)>>();
// std::vector<std::unique_ptr<star::StarBuffers::Buffer>> star::RenderResourceSystem::buffers =
//     std::vector<std::unique_ptr<star::StarBuffers::Buffer>>();

// void star::RenderResourceSystem::registerCallbacks(
//     std::function<void(star::core::device::DeviceContext &, const int &, const vk::Extent2D &)> initCallback,
//     std::function<void(star::core::device::DeviceContext &)> destroyCallback)
// {
//     initCallbacks.push(initCallback);
//     destroyCallbacks.push(destroyCallback);
// }

// void star::RenderResourceSystem::registerLoadGeomDataCallback(
//     std::function<std::pair<std::unique_ptr<StarBuffers::Buffer>, std::unique_ptr<StarBuffers::Buffer>>(
//         core::device::DeviceContext &, BufferHandle, BufferHandle)>
//         loadGeometryCallback)
// {
//     loadGeometryCallbacks.push(loadGeometryCallback);
// }

// void star::RenderResourceSystem::registerSetDrawInfoCallback(
...
// void star::RenderResourceSystem::init(core::device::DeviceContext &device, const int &numFramesInFlight,
...
//         std::function<std::pair<std::unique_ptr<star::StarBuffers::Buffer>, std::unique_ptr<star::StarBuffers::Buffer>>(
//             star::core::device::DeviceContext &, star::BufferHandle, star::BufferHandle)> &function =
//             loadGeometryCallbacks.top();
//         std::function<void(const uint32_t &, const uint32_t &, const uint32_t &)> &offsetFunction =
//             geometryDataOffsetCallbacks.top();
//         std::pair<std::unique_ptr<StarBuffers::Buffer>, std::unique_ptr<StarBuffers::Buffer>> result =
//             function(device, vertBufferHandle, indBufferHandle);
...
//     // 		device.copyBuffer(vertStage->getVulkanBuffer(), vertBuffer->getVulkanBuffer(), vertStage->getBufferSize(),
//     // currentOffset); 		currentOffset += vertStage->getBufferSize();
//     // 	}
//     // }
...
//     // 		device.copyBuffer(indStage->getVulkanBuffer(), indBuffer->getVulkanBuffer(), indStage->getBufferSize(),
//     // currentOffset);
...
// void star::RenderResourceSystem::runInits(core::device::DeviceContext &device, const int &numFramesInFlight,
//                                           const vk::Extent2D &screensize)
// {
//     while (!initCallbacks.empty())
//     {
//         std::function<void(core::device::DeviceContext &, const int &, const vk::Extent2D &)> &function =
//             initCallbacks.top();
...
// void star::RenderResourceSystem::bindBuffer(const uint32_t &bufferId, vk::CommandBuffer &commandBuffer,
//                                             const size_t &offset)
// {
