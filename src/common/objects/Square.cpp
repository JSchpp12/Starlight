...
//
//		vertStagingBuffer->writeToBuffer(verts->data(), sizeof(Vertex) * verts->size());
//
//		vertStagingBuffer->unmap(); 
//	}
//
//	std::unique_ptr<StarBuffer> indexStagingBuffer; 
//	{
//		indexStagingBuffer = std::make_unique<StarBuffer>(
//			device,
//			sizeof(uint32_t),
//			inds->size(),
//			vk::BufferUsageFlagBits::eTransferSrc,
...
//
//		indexStagingBuffer->writeToBuffer(inds->data(), sizeof(uint32_t) * inds->size());
//
//		indexStagingBuffer->unmap(); 
//	}
//
//	return std::pair<std::unique_ptr<StarBuffer>, std::unique_ptr<StarBuffer>>(std::move(vertStagingBuffer), std::move(indexStagingBuffer));
//}